...
def evaluation_inputs() -> tuple[dict[str, dict[str, Any]], float]:
    st.markdown("#### 🎯 Evaluador por Criterio de Desempeño")
    escala_40 = {"Experto (40 pts)": ("Experto", 40.0), "Capacitado (36 pts)": ("Capacitado", 36.0), "Aceptable (32 pts)": ("Aceptable", 32.0), "Aprendiz (28 pts)": ("Aprendiz", 28.0), "Requiere apoyo (24 pts)": ("Requiere apoyo", 24.0), "No evaluable (0 pts)": ("No evaluable", 0.0)}
    escala_20 = {"Experto (20 pts)": ("Experto", 20.0), "Capacitado (18 pts)": ("Capacitado", 18.0), "Aceptable (16 pts)": ("Aceptable", 16.0), "Aprendiz (14 pts)": ("Aprendiz", 14.0), "Requiere apoyo (12 pts)": ("Requiere apoyo", 12.0), "No evaluable (0 pts)": ("No evaluable", 0.0)}
    ...
    c1, c2 = st.columns(2)
    with c1:
        s_cog = st.selectbox("1. Cognitivo", list(escala_40.keys()), index=0); n_cog, p_cog = escala_40[s_cog]; criterios["Cognitivo"] = {"nivel": n_cog, "puntos": p_cog}; total += p_cog
        s_act = st.selectbox("2. Actitudinal", list(escala_20.keys()), index=0); n_act, p_act = escala_20[s_act]; criterios["Actitudinal"] = {"nivel": n_act, "puntos": p_act}; total += p_act
    with c2:
        s_com = st.selectbox("3. Comunicativo", list(escala_20.keys()), index=0); n_com, p_com = escala_20[s_com]; criterios["Comunicativo"] = {"nivel": n_com, "puntos": p_com}; total += p_com
        s_pen = st.selectbox("4. Pensamiento crítico", list(escala_20.keys()), index=0); n_pen, p_pen = escala_20[s_pen]; criterios["Pensamiento crítico"] = {"nivel": n_pen, "puntos": p_pen}; total += p_pen
