...
class RetroalimentacionApp:
    ...
    def run(self) -> None:
        ...
        elif pagina_actual == opciones_navegacion[3]: self.tab_ai_config()
        elif pagina_actual == opciones_navegacion[4]: self.tab_settings()
        elif pagina_actual == opciones_navegacion[5]: self.tab_forums()
    ...
    def tab_generate(self) -> None:
        ...
        if submit_eval:
            texto_base = observaciones_usuario if observaciones_usuario else ("Entregó en un formato equivocado." if formato_incorrecto else "")
                ...
                with st.expander(r["nombre"]):
                    ...
                    with st.form(f"form_edit_rub_{r['id']}"):
                        ...
                        if c1.form_submit_button("Actualizar"):
                            d = _Dummy(); d.nombre, d.contenido = e_nom, e_cont
                            self.db.update_rubric(r["id"], d); st.success("Rúbrica actualizada."); st.rerun()
            ...
            for a_raw in self.db.list_activities():
                ...
                with st.expander(f"⚙️ Editar: {getattr(act_obj, 'nombre', '')}"):
                    with st.form(f"form_edit_act_{act_obj.id}"):
                        ...
                        rub_val, fra_val = getattr(act_obj, "rubrica", None), getattr(act_obj, "frase", None)
                        ...
                        curr_recs_nombres = [getattr(r, "titulo", "") for r in getattr(act_obj, "recursos", []) if getattr(r, "titulo", "") in recurso_opts]
    ...
    def tab_settings(self) -> None:
        ...
        c1, c2 = st.columns(2)
        if c1.button("Crear respaldo", width="stretch"): path = self.db.backup(); st.success(f"Respaldo: {path.name}")
    ...
    def tab_forums(self) -> None:
        ...
        col1, col2 = st.columns(2)
        with col1: semana = st.selectbox("Semana del Módulo:", ["Semana 1", "Semana 2", "Semana 3", "Semana 4"])
        with col2: dia = st.selectbox("Día de participación:", ["Lunes (Apertura)", "Martes (Interacción)", "Miércoles (Redacción)", "Jueves (Matemática)", "Viernes (Cierre)"])
        tono = st.selectbox("Variación de estilo:", ["Estándar", "Empático", "Directo"])
        ...
        if st.button("✨ Generar Aportación Automática", type="primary", width="stretch"):
            ...
            temas = {"Semana 1": "Razones y proporciones.", "Semana 2": "Lenguaje común y algebraico.", "Semana 3": "Sistemas de ecuaciones.", "Semana 4": "Ecuaciones cuadráticas."}
            instrucciones_dia = {"Lunes (Apertura)": "Explica el propósito de la semana.", "Martes (Interacción)": "Fomenta el debate.", "Miércoles (Redacción)": "Fomenta buenas prácticas de redacción.", "Jueves (Matemática)": "Da una pista técnica.", "Viernes (Cierre)": "Concluye el foro."}
            
            prompt = f"""
            Eres {n_ase}, {r_ase} (Grupo {g_ase}).
            Redacta mi aportación diaria para el foro.
            Contexto: Tema {temas[semana]}. Día: {dia}. Instrucción: {instrucciones_dia[dia]}. Tono: {tono}.
            
            1. Saludo: "Apreciables estudiantes."
            2. Despedida OBLIGATORIA al final, EXACTAMENTE ASÍ:
            {n_ase}
            {r_ase}
            {id_ase}
            {g_ase}
            3. Escribe natural. No uses títulos Markdown (##).
            """
            with st.spinner("⏳ Redactando..."):
                try:
                    ...
                    st.text_area("Texto para Moodle:", value=respuesta, height=400)
                except Exception as e: st.error(f"Error: {e}")
