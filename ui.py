...
class RetroalimentacionApp:
    ...
    def run(self) -> None:
        ...
        if dirs_cache.get("asesor_nombre"):
            nombre_asesor = dirs_cache.get("asesor_nombre", "Asesor").split()[0]
        else:
    ...
    def tab_generate(self) -> None:
        ...
        if submit_eval:
            if observaciones_usuario:
                texto_base = observaciones_usuario
            elif formato_incorrecto:
                texto_base = "Entregó en un formato equivocado."
            else:
                texto_base = ""
            ...
            if modo == "👤 Individual":
                ...
                # Exclusión de Haiku en errores de formato
        ...
        if modo == "👤 Individual":
            if st.session_state.last_feedback:
                ...
                download_buttons(
                    ...
                    html_feedback,
                    docx_bytes("", st.session_state.last_feedback, n_ase, id_ase),
                    pdf_bytes("", st.session_state.last_feedback),
                    payload
                )
            ...
            if st.session_state.batch_queue:
                ...
                if st.button("🚀 Procesar todo el lote ahora", type="primary"):
                    ...
                    for idx, item in enumerate(st.session_state.batch_queue):
                        ...
                        modelo_usar = st.session_state.model_id
                        # Exclusión de Haiku en lote por error de formato
                        ...
                        try:
                            ...
                            retro = Retroalimentacion(
                                ...
                                texto_generado=text,
                                modelo_usado=st.session_state.model_name,
                                calificacion=b.calificacion,
                                criterios=b.criterios_evaluados,
    ...
    def tab_history(self) -> None:
        ...
        activities = {"Todas": None}
        for r in lista_actividades:
            activities[r["nombre"]] = r["id"]
        ...
        with st.expander("📦 Herramienta de Descarga en Lote (ZIP)", expanded=False):
            ...
            edited_df = st.data_editor(
                ...
                hide_index=True,
                disabled=["Fecha", "Estudiante", "Calificación", "ID"],
                width="stretch"
            )
            ...
            if st.button(f"📥 Descargar {len(selected_rows)} archivos en ZIP", type="primary", disabled=len(selected_rows)==0, width="stretch"):
                ...
                for r in selected_rows:
                    ...
                    if r.get("actividad_id") in act_map:
                        ...
                        act_val = r.get("actividad_nombre") or "General"
                ...
                if selected_act_name != "Todas":
                    act_str = get_activity_code(selected_act_name)
                else:
                    act_str = "Varias"
                    
                st.download_button(
                    "💾 Haz clic aquí para guardar tu archivo ZIP",
                    data=zip_bytes,
                    file_name=f"Retros_{grupo_zip}_{act_str}.zip",
    ...
    def tab_activities(self) -> None:
            ...
            for r in self.db.list_recursos_globales():
                ...
                with st.expander(f"📌 {r_tit} [{r_tip}]"):
                    with st.form(f"form_edit_rec_{r.id}"):
                        ...
                        opciones_tipo = ["Video", "Artículo", "Enlace", "PDF", "Otro"]
                        try:
                            idx_tipo = opciones_tipo.index(r_tip)
                        except ValueError:
                            idx_tipo = 0
                            
                        e_tip = st.selectbox("Tipo", opciones_tipo, index=idx_tipo)
        ...
        with t3:
            ...
            if mode == "Manual":
                rubrica, sub_rub = rubric_manual_form()
            else:
                rubrica, sub_rub = rubric_import_form()
            ...
            for a_raw in self.db.list_activities():
                ...
                with st.expander(f"⚙️ Editar: {getattr(act_obj, 'nombre', '')}"):
                    with st.form(f"form_edit_act_{act_obj.id}"):
                        ...
                        rubric_opts = {"Sin rúbrica": None}
                        for r in all_rubrics:
                            rubric_opts[r["nombre"]] = r["id"]
                            
                        frase_opts = {"Sin frase": None}
                        for f in all_frases:
                            texto_corto = str(getattr(f, "texto", ""))[:40]
                            autor_frase = str(getattr(f, "autor", ""))
                            frase_opts[f'"{texto_corto}..." - {autor_frase}'] = f.id
                            
                        recurso_opts = {}
                        for r in all_recursos:
                            recurso_opts[getattr(r, "titulo", "")] = r.id
                            
                        curr_recs_nombres = []
                        for r in getattr(act_obj, "recursos", []):
                            titulo_r = getattr(r, "titulo", "")
                            if titulo_r in recurso_opts:
                                curr_recs_nombres.append(titulo_r)

                        c1, c2 = st.columns(2)
                        
                        try:
                            idx_rub = list(rubric_opts.values()).index(rub_id)
                        except ValueError:
                            idx_rub = 0
                            
                        try:
                            idx_fra = list(frase_opts.values()).index(fra_id)
                        except ValueError:
                            idx_fra = 0

                        e_rub = c1.selectbox("Rúbrica asociada", list(rubric_opts.keys()), index=idx_rub)
                        e_fra = c2.selectbox("Frase de cierre", list(frase_opts.keys()), index=idx_fra)
                        ...
                        if col1.form_submit_button("Actualizar Ensamblado"):
                            grupo_val = getattr(act_obj, "grupo", "M00C0G00-000")
                            orden_val = getattr(act_obj, "orden", 0)
                            ids_recursos_seleccionados = [recurso_opts[n] for n in e_recs if n in recurso_opts]
    ...
    def tab_ai_config(self) -> None:
        ...
        with st.form("form_perfil_y_prompts"):
            ...
            if st.form_submit_button("Guardar Perfil e Instrucciones", type="primary", use_container_width=True):
                ...
                self.db.update_directriz("error_formato", d_formato_err)
